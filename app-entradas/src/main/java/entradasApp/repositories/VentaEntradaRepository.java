...
public interface VentaEntradaRepository extends JpaRepository<VentaEntrada, Long> {
    ...
    @Query(value = "SELECT SUM(CASE WHEN ve.compradorEntrada.paseDeOro = true AND ve.entrada.juego.paseDeOro = true THEN 0 ELSE ve.montoVenta END) FROM VentaEntrada ve WHERE DATE(ve.fechaVenta) = :fecha", nativeQuery = true)
